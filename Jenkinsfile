pipeline {
  ...
  environment {
    ...
    EXT_PIP = 'yq'
    BUILD_VERSION_ARG = 'YQ_VERSION'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-yq'
    CONTAINER_NAME = 'yq'
    DOCKERHUB_IMAGE = 'linuxserver/yq'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/yq'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/yq") | .uuid' || :)
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/yq",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-yq",\
                       "backendUrl":"https://ghcr.io/linuxserver/yq",\
                       "publicUrl":"https://lscr.io/linuxserver/yq"}' || :
              else
